...
class Minesweeper():
    ...
    def __init__(self, height=8, width=8, mines=8):
        ...
        while len(self.mines) != mines:
            ...
            if not self.board[i][j]:
                self.mines.add((i, j))
                self.board[i][j] = '_'  # Represent mines as 'T'
        
        self.mines_found = set()
        
        # Update board with count of neighboring mines
        for i in range(self.height):
            for j in range(self.width):
                if not self.board[i][j]:
                    count = self.nearby_mines((i, j))
                    self.board[i][j] = count
                    
        self.write_board_to_file('board.txt')
    ...
    def print(self):
        ...
        for i in range(self.height):
            print("--" * self.width + "-")
            for j in range(self.width):
                if self.board[i][j] == '_':  # Represent mines as '_'
                    print("|_", end="")
                else:
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if self.board[i][j] == '_':  # Represent mines as 'T'
                        count += 1
    ...
    def write_board_to_file(self, filename):
        with open(filename, 'w') as f:
            for row in self.board:
                # Replace 0 with '_' and join elements with commas
                row_str = ','.join(['_' if elem == 0 else str(elem) for elem in row])
                f.write(row_str + '\n')
